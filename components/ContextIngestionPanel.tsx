...
// Mounted in app/mirror/page.tsx (and, in a locked-only form, referenced from
...
import { parseExportFile, parsePlainText } from '@/lib/context-export-parser'
import type {
  ContextIngestionStatusResponse, UserMemoryFact, MemoryFactCategory,
} from '@/lib/types'
...
export default function ContextIngestionPanel({ authToken }: Props) {
  ...
  const fetchStatus = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) { setLoadingStatus(false); return }
  ...
  async function runSubmit(mode: 'chatgpt' | 'claude' | 'manual' | 'pasted_summary', text: string) {
    setSubmitError(null)
    setSubmitting(true)
    setProgressStep(0)
    ...
    try {
      ...
      await new Promise(r => setTimeout(r, 500))   // let the last tick be visible
  ...
  async function handleFileChosen(file: File) {
    ...
    try {
      ...
      await runSubmit(parsed.sourceType as 'chatgpt' | 'claude', parsed.text)
    } catch (err) {
  ...
  async function handleSaveReview() {
    ...
    try {
      ...
      if (res.ok) { setReviewFacts([]); await fetchStatus() }
  ...
  if (submitting) {
    return (
      <div style={cardStyle()}>
        ...
        <h3 style={heading()}>{STEPS[progressStep]}…</h3>
  ...
  if (status === 'review_pending' && reviewFacts.length > 0) {
    return (
      <div style={cardStyle()}>
        ...
        <button type="button" onClick={handleSaveReview} disabled={saving} className="btn-primary" style={{ width: '100%', fontSize: 14, padding: '12px' }}>
          {saving ? 'Saving…' : `Save ${Object.values(checked).filter(Boolean).length} insight${Object.values(checked).filter(Boolean).length === 1 ? '' : 's'} →`}
        </button>
      </div>
    )
  }
      ...
      <div style={cardStyle()}>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <button type="button" onClick={handleReanalyze} disabled={busyAction !== null}
            ...
            {busyAction === 'reanalyze' ? 'Refreshing…' : 'Refresh with latest model'}
          </button>
  ...
  // ── Render: start screen (no ingestion yet, or failed/forgotten) ─────────────
  return (
    <div style={cardStyle()}>
      <p style={eyebrow('var(--gold)')}>Elite · Context Import</p>
      <h3 style={heading()}>Accelerate your Council by teaching Quorum who you are</h3>
      ...
      {inputMode === 'text' ? (
        ...
        <>
          <input
            ref={fileInputRef} type="file" accept=".zip,.json" style={{ display: 'none' }}
          ...
          <button type="button" onClick={() => fileInputRef.current?.click()} className="btn-primary" style={{ width: '100%', fontSize: 14, padding: '11px' }}>
            Choose ChatGPT or Claude export →
          </button>
          <p style={{ fontSize: 10.5, color: 'var(--text-4)', marginTop: 8 }}>
            Your export is parsed in your browser — the original file never leaves your device. Only the extracted insights below are sent.
          </p>
        </>
      )}
    </div>
